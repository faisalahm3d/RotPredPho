...
def plot_roc_curves(fpr, tpr, roc_auc, label,protein_site):
    ...
    plt.savefig('graphs/roc_curve'+protein_site+'.png', dpi=500)
    plt.show()
...
tpr = dict()
roc_auc = dict()
label = ('combined', 'evolutionary', 'structural')


for i in range(len(label)):
    model_path = 'trained_models/'+label[i]+'_p'+protein_site+'.pkl'
    modelfile = open(model_path, 'rb')
    model = pickle.load(modelfile)
    if label[i] == 'combined':
        test_x = x_test
    elif label[i] == 'evolutionary':
        test_x = x_test[:, :400]
    elif label[i] =='structural':
        test_x = x_test[:, 400:]
    y_test_predict = model.predict(test_x)
    y_test_predict_prob = model.predict_proba(test_x)
