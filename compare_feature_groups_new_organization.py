...
def cross_val(x_train, y_train, x_test, y_test, folds, feature_group):
    ...
    with open('trained_models/{}_p{}.pkl'.format(feature_group,residue_name), 'wb') as f:
        pickle.dump(model, f)

    y_test_predict = model.predict(x_test)
    y_test_predict_prob = model.predict_proba(x_test)

    res = "                        Experimental result with {} features\n".format(feature_group)
    res += "                 (window size 10 is used with rotation forest classifier)\n"
    res += "-----------------------------------------------------------------------------------------------------------------------------\n"
    res += "                   {} folds CV          independent test                   {} folds CV          independent test\n".format(folds)
    res += "-----------------------------------------------------------------------------------------------------------------------------\n"
    res += "Accuracy        Sensitivity    Specificity    MCC     AUC       "
    res += "Accuracy:      {0:0.5f}({1:0.5f})          {2:0.5f}                      {0:0.5f}({1:0.5f})          {2:0.5f}\n".format(np.mean(accuracy), np.std(accuracy), accuracy_score(y_test, y_test_predict))
    res += "MCC:           {0:0.5f}({1:0.5f})          {2:0.5f}                      {0:0.5f}({1:0.5f})          {2:0.5f}\n".format(np.mean(mcc), np.std(mcc), matthews_corrcoef(y_test, y_test_predict))
    res += "Precision:     {0:0.5f}({1:0.5f})          {2:0.5f}                      {0:0.5f}({1:0.5f})          {2:0.5f}\n".format(np.mean(precision), np.std(precision), precision_score(y_test, y_test_predict))
...
if __name__ == '__main__':
    ...
    for feature_name in feature_group:
        train_x = feature_group[feature_name]['x_train']
        test_x = feature_group[feature_name]['x_test']
        cross_val(train_x, y_train, test_x, y_test, 5, feature_name)
